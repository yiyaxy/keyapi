...
func PreWssConsumeQuota(ctx *gin.Context, relayInfo *relaycommon.RelayInfo, usage *dto.RealtimeUsage) error {
	...
	token, err := model.GetTokenByKeyWithContext(ctx, strings.TrimPrefix(relayInfo.TokenKey, "sk-"), false)
	if err != nil {
		return err
	}
...
func PostWssConsumeQuota(ctx *gin.Context, relayInfo *relaycommon.RelayInfo, modelName string,
	...
	// Apply channel-specific and other ratios
	if len(relayInfo.PriceData.OtherRatios) > 0 {
		otherMultiplier := 1.0
		for _, ratio := range relayInfo.PriceData.OtherRatios {
			otherMultiplier *= ratio
		}
		quota = int(float64(quota) * otherMultiplier)
	}
	...
	if totalTokens == 0 {
		...
		model.UpdateUserUsedQuotaAndRequestCount(relayInfo.UserId, quota, relayInfo.TenantId)
...
func PreConsumeTokenQuota(relayInfo *relaycommon.RelayInfo, quota int) error {
	...
	//	return nil
	//}
	tokenCtx := context.WithValue(context.Background(), constant.ContextKeyTenantId, relayInfo.TenantId)
	token, err := model.GetTokenByKeyWithContext(tokenCtx, relayInfo.TokenKey, false)
	if err != nil {
		return err
	}
	...
	err = model.DecreaseTokenQuota(relayInfo.TokenId, relayInfo.TokenKey, quota, relayInfo.TenantId)
	if err != nil {
		return err
	}
	return nil
...
func PostConsumeQuota(relayInfo *relaycommon.RelayInfo, quota int, preConsumedQuota int, sendEmail bool) (err error) {
	...
	if relayInfo != nil && relayInfo.BillingSource == BillingSourceSubscription {
		...
		// Wallet
		if quota > 0 {
			err = model.DecreaseUserQuota(relayInfo.UserId, quota, relayInfo.TenantId)
		} else {
			err = model.IncreaseUserQuota(relayInfo.UserId, -quota, false, relayInfo.TenantId)
		}
		if err != nil {
			return err
		}
	...
	if !relayInfo.IsPlayground {
		if quota > 0 {
			...
			err = model.IncreaseTokenQuota(relayInfo.TokenId, relayInfo.TokenKey, -quota, relayInfo.TenantId)
		}
		if err != nil {
			return err
		}
...
<p style="margin:16px 0">为了不影响您的正常使用，请及时充值。</p>
<div style="margin:24px 0;text-align:center">
<a href="%s" style="display:inline-block;padding:12px 32px;background:linear-gradient(135deg,#4f46e5,#7c3aed);color:#ffffff;text-decoration:none;border-radius:8px;font-size:15px;font-weight:600">立即充值</a>
</div>`, logger.FormatQuota(relayInfo.UserQuota), topUpLink))
				values = nil
			}

...
func UpdateTenantMember(c *gin.Context) {
	...
	roleBefore, statusBefore := 0, 0
	if before, err := model.GetTenantMembershipUnscoped(tenantId, req.UserId); err == nil && before != nil {
	...
	service.RecordAudit(c, "membership.update", "user", req.UserId, gin.H{

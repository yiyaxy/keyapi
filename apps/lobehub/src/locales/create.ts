...
// locale JSON files so dev-preview JSON can still customize bundled copy
...
const createBundledResources = (locale = DEFAULT_LANG) => ({
  chat: mergeNamespace(defaultChat, locale === 'zh-CN' ? zhChat : enChat),
  common: mergeNamespace(defaultCommon, locale === 'zh-CN' ? zhCommon : enCommon),
  error: mergeNamespace(defaultError, locale === 'zh-CN' ? zhError : enError),
});
    ...
    init: (params: { initAsync?: boolean } = {}) => {
      ...
      const bundledLanguageResources =
        initialLang === DEFAULT_LANG
          ...
          : {
              [DEFAULT_LANG]: defaultResources,
              [initialLang]: createBundledResources(initialLang),
            };
      ...
      const initPromise = instance.init({
        ...
        initAsync,
        // Keep init synchronous so components can render with bundled resources
        // before the user's actual language finishes loading in the background.
        ns: [],

        // Preload default language synchronously to avoid Suspense on first render
        resources: {
        ...
        // so preloaded fallback gets replaced by the user's actual language.
        react: {
          bindI18nStore: 'added',
          useSuspense: false,
        },

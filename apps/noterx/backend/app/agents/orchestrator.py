...
class Orchestrator:
    ...
    async def run(
        ...
        t1 = time.time()
        content_agent = ContentAgent(model=get_model_pro())
        visual_agent = VisualAgent(model=get_model_pro())
        growth_agent = GrowthAgent(model=get_model_pro())
        user_sim_agent = UserSimAgent(model=get_model_pro())
        ...
        judge = JudgeAgent(model=get_model_pro())
            ...
            try:
                result = await asyncio.wait_for(
                    agents[idx].call_llm(
                        ...
                        model_override=get_model_fast(), max_tokens=1024,
                    ),

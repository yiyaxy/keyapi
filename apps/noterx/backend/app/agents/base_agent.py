...
# Dynamic LLM context variables for platform integration
...
def _is_mimo_openai_compat() -> bool:
    ...
    if os.getenv("OPENAI_COMPAT", "").strip().lower() == "mimo":
        return True
    base = (llm_base_url_var.get() or os.getenv("OPENAI_BASE_URL") or "").lower()
    if "xiaomimimo.com" in base or "mimo-v2.com" in base:
        return True
    model = (llm_model_var.get() or os.getenv("LLM_MODEL") or "").lower()
...
def _resolve_openai_base_url() -> Optional[str]:
    ...
    """
    if llm_base_url_var.get():
        raw = llm_base_url_var.get().strip()
    elif llm_api_key_var.get():
        proxy_url = (os.getenv("OPENAI_PROXY_URL") or "").strip()
        new_api_base = (os.getenv("NEW_API_BASE_URL") or "").strip().rstrip("/")
        raw = (
            proxy_url
            or (f"{new_api_base}/v1" if new_api_base else "")
...
def _allow_env_llm_key() -> bool:
    return os.getenv("NOTERX_ALLOW_ENV_LLM_KEY", "").strip().lower() in ("1", "true", "yes")


def _resolve_openai_api_key() -> str:
    request_key = (llm_api_key_var.get() or "").strip()
    if request_key:
        return request_key
    if _allow_env_llm_key():
...
class BaseAgent:
    ...
    def __init__(self, model: Optional[str] = None):
        self.model = llm_model_var.get() or model or get_model_pro()

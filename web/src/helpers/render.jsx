...
export function getLucideIcon(key, selected = false) {
  ...
  switch (key) {
    ...
    case 'analytics':
      return <BarChart2 {...commonProps} color={iconColor} />;
    case 'purchase':
    ...
    case 'promptRule':
      return <Replace {...commonProps} color={iconColor} />;
    case 'ipAnalysis':
 ...
 */
export function convertUSDToCurrency(usdAmount, digits = 6) {
...
export function getSmartCacheSavingsQuota(
  otherPayload,
  promptTokens = 0,
  completionTokens = 0,
  loggedQuota = 0,
) {
  let other = otherPayload;
  if (typeof otherPayload === 'string') {
    try {
      other = JSON.parse(otherPayload || '{}');
    } catch (e) {
      other = {};
    }
  }
  ...
  if (!Number.isFinite(cacheTokens) || cacheTokens <= 0) {
    return 0;
  }
  ...
  const groupRatio = Number(other.group_ratio || 0);
  const userGroupRatio = Number(other.user_group_ratio || 0);
  const effectiveGroupRatio =
    Number.isFinite(userGroupRatio) && userGroupRatio > 0 && userGroupRatio !== -1
      ? userGroupRatio
      : groupRatio;
  if (!Number.isFinite(effectiveGroupRatio) || effectiveGroupRatio <= 0) {
    return 0;
  }

  const baseInputRatio = modelRatio * effectiveGroupRatio;
  const cacheRatio = Number(other.cache_ratio || 0);
  if (!Number.isFinite(cacheRatio) || cacheRatio < 0) {
    return 0;
  }

  let savings = cacheTokens * baseInputRatio * (1 - cacheRatio);
  const channelRatio = Number(other.channel_ratio || 0);
  if (
    Number.isFinite(channelRatio) &&
    channelRatio > 0 &&
    channelRatio < 1
  ) {
    const quotaValue = Number(loggedQuota || 0);
    let fullQuotaBeforeChannelDiscount = quotaValue / channelRatio;
    if (
      !Number.isFinite(fullQuotaBeforeChannelDiscount) ||
      fullQuotaBeforeChannelDiscount <= 0
    ) {
      fullQuotaBeforeChannelDiscount =
        baseInputRatio * Number(promptTokens || 0) +
        baseInputRatio * cacheTokens * cacheRatio;
      const completionRatio = Number(other.completion_ratio || 0);
      if (Number.isFinite(completionRatio) && completionRatio > 0) {
        fullQuotaBeforeChannelDiscount +=
          baseInputRatio * completionRatio * Number(completionTokens || 0);
      }
    }
    if (
      Number.isFinite(fullQuotaBeforeChannelDiscount) &&
      ...
      savings += fullQuotaBeforeChannelDiscount * (1 - channelRatio);
    }
  }
...
export function renderSmartCacheQuota(quota) {
  return renderQuota(Number(quota) || 0, 3);
}

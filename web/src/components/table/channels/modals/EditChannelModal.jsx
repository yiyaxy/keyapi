...
function type2secretPrompt(type) {
  ...
  switch (type) {
    ...
    case 50:
      return '按照如下格式输入: AccessKey|SecretKey, 如果上游是CaMeL API，则直接输ApiKey';
    case 51:
...
const EditChannelModal = (props) => {
  ...
  const originInputs = {
    ...
    cache_billing_mode: 'auto',
    channel_ratio: 1,
    model_ratio_override: {},
    // 虚拟缓存（UI 使用百分比）
    virtual_cache_enabled: false,
    virtual_cache_hit_rate: 50,
    virtual_cache_jitter: 5,
    settings: '',
  ...
  const handleInputChange = (name, value) => {
    ...
    if (name === 'base_url' && value.endsWith('/v1')) {
      Modal.confirm({
        title: '警告',
        content:
          '不需要在末尾加/v1，CaMeL API会自动处理，添加后可能导致请求失败，是否继续？',
        onOk: () => {
    ...
    if (success) {
      ...
      if (data.setting) {
        try {
          ...
          data.cache_billing_mode =
            parsedSettings.cache_billing_mode || 'auto';
          data.channel_ratio = parsedSettings.channel_ratio || 1;
          data.model_ratio_override = parsedSettings.model_ratio_override
            ? JSON.stringify(parsedSettings.model_ratio_override, null, 2)
            : '';

          data.virtual_cache_enabled =
            parsedSettings.virtual_cache_enabled === true;
          const virtualHitRate = parsedSettings.virtual_cache_hit_rate;
          data.virtual_cache_hit_rate =
            typeof virtualHitRate === 'number' && !Number.isNaN(virtualHitRate)
              ? virtualHitRate * 100
              : 50;
          const virtualJitter = parsedSettings.virtual_cache_jitter;
          data.virtual_cache_jitter =
            typeof virtualJitter === 'number' && !Number.isNaN(virtualJitter)
              ? virtualJitter * 100
              : 5;
        } catch (error) {
          ...
          data.cache_billing_mode = 'auto';
          data.channel_ratio = 1;
          data.model_ratio_override = '';
          data.virtual_cache_enabled = false;
          data.virtual_cache_hit_rate = 50;
          data.virtual_cache_jitter = 5;
        }
      } else {
        data.force_format = false;
        data.thinking_to_content = false;
      ...
      setChannelSettings({
        ...
        cache_billing_mode: data.cache_billing_mode || 'auto',
      });
  ...
  const submit = async () => {
    ...
    const channelExtraSettings = {
      ...
      cache_billing_mode: localInputs.cache_billing_mode || 'auto',
      channel_ratio: localInputs.channel_ratio || 1,
      virtual_cache_enabled: localInputs.virtual_cache_enabled === true,
      virtual_cache_hit_rate:
        Number(localInputs.virtual_cache_hit_rate || 50) / 100,
      virtual_cache_jitter: Number(localInputs.virtual_cache_jitter || 5) / 100,
    };
    // Parse model_ratio_override: may be a JSON string from TextArea or an object
    let modelRatioOverride = localInputs.model_ratio_override;
    if (typeof modelRatioOverride === 'string' && modelRatioOverride.trim()) {
      try {
        modelRatioOverride = JSON.parse(modelRatioOverride);
      } catch (e) {
        console.error('model_ratio_override JSON parse error:', e);
        modelRatioOverride = {};
      }
    }
    if (modelRatioOverride && typeof modelRatioOverride === 'object' && Object.keys(modelRatioOverride).length > 0) {
      channelExtraSettings.model_ratio_override = modelRatioOverride;
    }
    ...
    delete localInputs.cache_billing_mode;
    delete localInputs.channel_ratio;
    delete localInputs.model_ratio_override;
    delete localInputs.virtual_cache_enabled;
    delete localInputs.virtual_cache_hit_rate;
    delete localInputs.virtual_cache_jitter;
              ...
              <div className='space-y-4'>
                ...
                <div className='pt-3'>
                  ...
                  <Form.InputNumber
                    ...
                    step={0.1}
                    min={0.1}
                    extraText={t('渠道计费倍率说明')}
                    onChange={(value) =>
                      handleChannelSettingsChange('channel_ratio', value)
                    }
                    style={{ width: '100%' }}
                  />

                  <Form.TextArea
                    field='model_ratio_override'
                    label={t('模型倍率覆盖')}
                    placeholder={t('模型倍率覆盖示例')}
                    extraText={t('模型倍率覆盖说明')}
                    onChange={(value) =>
                  ...
                  <Form.Select
                    field='cache_billing_mode'
                    label={t('缓存计费模式')}
                    initValue='auto'
                    onChange={(value) =>
                      handleChannelSettingsChange('cache_billing_mode', value)
                    }
                    optionList={[
                      {
                        label: t('自动（跟随上游语义）'),
                      ...
                      {
                        label: t('包含缓存 tokens（inclusive）'),
                        value: 'inclusive',
                      },
                      {
                        label: t('不包含缓存 tokens（exclusive）'),
                        value: 'exclusive',
                      },
                    ]}
                    extraText={t(
                      'auto 会按上游语义自动判断；inclusive 表示 prompt_tokens 已包含 cache tokens，结算时会先扣除缓存 tokens 再按缓存倍率加回；exclusive 表示 prompt_tokens 不包含 cache tokens，结算时不会扣除缓存 tokens。',
                    )}
                  />

                  <Form.Switch
                    field='virtual_cache_enabled'
                    label={t('虚拟缓存')}
                    checkedText={t('开')}
                    uncheckedText={t('关')}
                    onChange={(value) =>
                      handleChannelSettingsChange('virtual_cache_enabled', value)
                    }
                    extraText={t('虚拟缓存说明')}
                  />

                  <Form.InputNumber
                    field='virtual_cache_hit_rate'
                    label={t('虚拟缓存命中率 (%)')}
                    min={1}
                    max={99}
                    step={1}
                    disabled={inputs.virtual_cache_enabled !== true}
                    onNumberChange={(value) =>
                      handleChannelSettingsChange('virtual_cache_hit_rate', value)
                    }
                    style={{ width: '100%' }}
                  />

                  <Form.InputNumber
                    field='virtual_cache_jitter'
                    label={t('虚拟缓存抖动 (%)')}
                    min={0}
                    max={20}
                    step={1}
                    disabled={inputs.virtual_cache_enabled !== true}
                    onNumberChange={(value) =>
                      handleChannelSettingsChange('virtual_cache_jitter', value)
                    }
                    style={{ width: '100%' }}
                  />
              ...
              <div className='p-2 space-y-3' ref={formContainerRef}>
                ...
                <Card className='!rounded-2xl shadow-sm border-0'>
                      ...
                      {inputs.type === 8 && (
                        <>
                          <Banner
                            type='warning'
                            description={t(
                              '如果你对接的是上游CaMeL API等转发项目，请使用OpenAI类型，不要使用此类型，除非你知道你在做什么。',
                            )}
                            className='!rounded-lg'
                          />
                          <div>
                        ...
                        (inputs.type !== 45 || doubaoApiEditUnlocked) && (
                          <div>
                            <Form.Input
                              ...
                              disabled={isIonetLocked}
                              extraText={t(
                                '对于官方渠道，CaMeL API已经内置地址，除非是第三方代理站点或者Azure的特殊接入地址，否则不需要填写',
                              )}
                            />
                          </div>
                        )}
                  ...
                  <Form.InputNumber
                    ...
                    min={0}
                    max={10}
                    extraText={t('失败后在同渠道重试的次数')}
                    onChange={(value) =>
                      handleInputChange('max_retry', value)
                    }
                  />

...
describe('integration: keys edit optimistic', () => {
  test('save updates table before PUT resolves', async () => {
    ...
    await user.click(await screen.findByRole('menuitem', { name: /Edit|编辑/ }));
  ...
  test('toggle failure shows backend error toast', async () => {
    server.use(
      http.get('/api/user/self/channel-groups', () =>
        HttpResponse.json({
          success: true,
      ...
      http.put('/api/token/', () =>
        HttpResponse.json({
          success: false,
          message: 'tenant guardrail: refusing unscoped UPDATE on tokens',
        })
      )
    );
    const qc = new QueryClient({ defaultOptions: { queries: { retry: false } } });
    ...
    await waitFor(() => expect(screen.getByText('original')).toBeInTheDocument());
    await user.click(screen.getByRole('button', { name: /Actions|操作/ }));
    await user.click(await screen.findByRole('menuitem', { name: /disabled|禁用/i }));
    await waitFor(() =>
      expect(
        screen.getByText('tenant guardrail: refusing unscoped UPDATE on tokens')
      ).toBeInTheDocument()
    );
  });
});
